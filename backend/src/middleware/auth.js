...
if (!process.env.JWT_SECRET) {
  throw new Error(
    "JWT_SECRET es requerido. Configurar en variables de entorno.",
  );
}
...
export const authenticateToken = (req, res, next) => {
  try {
    ...
    req.user = {
      ...
      rol:
        decoded.tipo_usuario === "administrador"
          ? "admin"
...
export const checkUserActive = async (req, res, next) => {
  try {
    ...
    const isActiveAndVerified = await userModel.isUserActiveAndVerified(
      req.user.id,
    );
...
export const checkClientAccess = async (req, res, next) => {
  try {
    ...
    if (
      req.user.tipo_usuario === "administrador" ||
      req.user.tipo_usuario === "empleado"
    ) {
      return next();
    }
