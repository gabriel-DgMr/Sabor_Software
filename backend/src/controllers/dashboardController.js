...
export const dashboardController = {
  ...
  async getMetrics(req, res) {
    try {
      ...
      // Total de usuarios por día (últimos 14 días)
  ...
  async getSalesMetrics(req, res) {
    try {
      ...
      // Ventas totales (considerando estados 3 y 5 como completados/validados)
  ...
  async getEmployeeMetrics(req, res) {
    try {
      ...
      // Total empleados (roles 2 y 3 considerados empleados)
      ...
      // Nota: ajustar columna 'last_active' si tu DB usa otro nombre (ej. ultimo_acceso)
      ...
      const empleados_regulares = empleadosRegularesResult[0]?.empleados_regulares || 0;
      console.log("Empleados regulares:", empleados_regulares);

      // Empleados por rol (nombre del rol en tabla roles)
