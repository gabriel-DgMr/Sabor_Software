...
export const getAllUsers = async (req, res) => {
  try {
    ...
    const users = await userModel.getRecentUsers(
...
export const searchEmpleados = async (req, res) => {
  try {
    ...
    const filteredEmpleados = allEmpleados.filter(
      (empleado) =>
...
export const getEmpleadoStats = async (req, res) => {
  try {
    const stats = await userModel.getUserStats();
    const empleadoStats = stats.filter(
      (stat) =>
        stat.tipo_usuario === "empleado" ||
        stat.tipo_usuario === "administrador",
    );

    const totalStats = empleadoStats.reduce(
      (acc, stat) => ({
