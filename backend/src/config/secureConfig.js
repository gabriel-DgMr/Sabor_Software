...
export const secureConfig = {
  ...
  rateLimit: {
    ...
    register: {
      windowMs: parseInt(process.env.REGISTER_RATE_LIMIT_WINDOW, 10) || 60 * 60 * 1000,
      max: parseInt(process.env.REGISTER_RATE_LIMIT_MAX, 10) || 3
    },
        
    // Rate limiting para recuperación de contraseña
    passwordReset: {
      windowMs: parseInt(process.env.PASSWORD_RESET_RATE_LIMIT_WINDOW, 10) || 60 * 60 * 1000,
  ...
  files: {
    ...
    allowedMimeTypes: (process.env.FILE_ALLOWED_TYPES || 'image/jpeg,image/jpg,image/png,image/webp').split(','),
  ...
  validation: {
    ...
    email: {
      ...
      allowedDomains: process.env.EMAIL_ALLOWED_DOMAINS ? process.env.EMAIL_ALLOWED_DOMAINS.split(',') : null
    },
