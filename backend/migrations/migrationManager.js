...
class MigrationManager {
  ...
  async executeStatement(statement) {
    ...
    const requiresQuery = unsupportedCommands.some((cmd) =>
      trimmedStatement.startsWith(cmd),
    );
  ...
  async initializeMigrationTable() {
    try {
      const migrationControlPath = path.join(
        this.migrationsPath,
  ...
  async getMigrationFiles() {
    try {
      ...
      return files
        .filter(
          (file) =>
            file.endsWith(".sql") && file !== "000_migration_control.sql",
        )
        .sort();
    } catch (error) {
  ...
  async executeMigration(migrationFile) {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Error al registrar error en migration_history:",
          logError,
        );
      }
  ...
  async showMigrationStatus() {
    try {
      ...
      console.log(
        `Pendientes: ${migrationFiles.length - executedMigrations.length}`,
      );
    } catch (error) {
