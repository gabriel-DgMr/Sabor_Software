...
class AuditPartitionManager {
    ...
    async showPartitionInfo() {
        ...
        partitions.forEach(partition => {
            console.log(`🗂️  ${partition.partition_name} | ${partition.partition_month} | ${partition.table_rows} rows | ${partition.size_mb} MB`);
        });
    }
    ...
    async showEvents() {
        ...
        events.forEach(event => {
            ...
            console.log(`   ⏱️  Frecuencia: Cada ${event.interval_value} ${event.interval_field}`);
    ...
    async testAuditLog() {
        ...
        try {
            ...
            if (auditLogs.length > 0) {
                ...
                console.log(`🗂️  Partición: ${await this.getPartitionForDate(auditLogs[0].changed_at)}`);
            } else {
