...
class RailwayInitializer {
  ...
  async ensureOptionalTables() {
    appLogger.info("🧱 Verificando tablas opcionales...");
    ...
    try {
      ...
      appLogger.info("✅ Tabla opcional calificaciones_productos verificada");
    } finally {
      await connection.end();
    }
  }
  ...
  async run() {
    try {
      ...
      if (isRailway) {
        ...
        // Crear tablas opcionales que pueden faltar en entornos ya existentes
        await this.ensureOptionalTables();
      }
