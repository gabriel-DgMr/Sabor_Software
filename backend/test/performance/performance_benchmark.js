...
class PerformanceBenchmark {
  ...
  async generateTestData(recordCount = 1000) {
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      try {
        ...
        if (batch % 10 === 0) {
          console.log(
            `   Batch ${batch + 1}/${batches} - ${insertedRecords} records inserted`,
          );
        }
      } catch (error) {
        console.error(
          `❌ Error inserting batch ${batch + 1}:`,
    ...
    console.log(
      `✅ Test data generation completed: ${insertedRecords} records in ${totalTime.toFixed(2)}ms`,
    );
  ...
  async runPartitionInfoTest() {
    ...
    for (const test of tests) {
      try {
        const { results: queryResults, executionTime } =
          await this.executeQuery(test.query, test.params || []);
        results.push({
          name: test.name,
          executionTime,
        ...
        console.log(
          `   ❌ ${test.name}: ${error.error?.message || error.message}`,
        );
      }
    }
  ...
  async runQueryPerformanceTests() {
    ...
    for (const test of tests) {
      try {
        ...
        console.log(
          `   ✅ ${test.name}: ${executionTime.toFixed(2)}ms (${Array.isArray(queryResults) ? queryResults.length : 0} records)`,
        );
      } catch (error) {
        results.push({
          name: test.name,
  ...
  async runLoadTest(concurrentQueries = 5, queriesPerConnection = 10) {
    ...
    for (let i = 0; i < concurrentQueries; i++) {
      const connectionPromise = this.runConnectionQueries(
        query,
    ...
    try {
      ...
      const avgQueryTime =
        connectionResults.reduce((sum, conn) => sum + conn.avgTime, 0) /
        concurrentQueries;
      const minTime = Math.min(
        ...connectionResults.map((conn) => conn.minTime),
      );
      const maxTime = Math.max(
        ...connectionResults.map((conn) => conn.maxTime),
      );
      const successfulQueries = connectionResults.reduce(
        (sum, conn) => sum + conn.successful,
        0,
      );
      const failedQueries = connectionResults.reduce(
        (sum, conn) => sum + conn.failed,
        0,
      );
      ...
      console.log(
        `   Queries per second: ${((totalQueries / totalTime) * 1000).toFixed(2)}`,
      );
  ...
  async saveResults(filename = "performance_results.json") {
    ...
    const report = {
      ...
      summary: {
        ...
        totalExecutionTime: this.results.reduce(
  ...
  async runFullBenchmark() {
    ...
    try {
      ...
      console.log(
        `Overall success: ${this.results.every((r) => r.success) ? "✅" : "❌"}`,
      );
