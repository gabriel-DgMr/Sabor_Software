...
export const validarRegistro = (datos) => {
    ...
    const camposRequeridos = ['nombre_cliente', 'email_cliente', 'telefono_cliente', 'contraseña_cliente'];
    camposRequeridos.forEach(campo => {
        const error = validarCampoRequerido(datos[campo], campo.replace('_cliente', ''));
        if (error) errores[campo] = error;
    });
    
    if (Object.keys(errores).length > 0) return errores;
    
    const longitudError = validarLongitud(datos.nombre_cliente, 'nombre', 2, 50);
    if (longitudError) errores.nombre_cliente = longitudError;
    
    const emailLongitudError = validarLongitud(datos.email_cliente, 'email', 5, 100);
    if (emailLongitudError) errores.email_cliente = emailLongitudError;
    
    const emailError = validarEmail(datos.email_cliente);
    if (emailError) errores.email_cliente = emailError;
    
    const telefonoError = validarTelefono(datos.telefono_cliente);
    if (telefonoError) errores.telefono_cliente = telefonoError;
    
    const contraseñaError = validarContraseña(datos.contraseña_cliente);
    if (contraseñaError) errores.contraseña_cliente = contraseñaError;
    
    // Validación de espacios personalizada
    const caracteresErrorNombre = validarCaracteresEspeciales(datos.nombre_cliente, 'nombre');
    if (caracteresErrorNombre) errores.nombre_cliente = caracteresErrorNombre;
    const espaciosErrorNombre = validarEspacios(datos.nombre_cliente, 'nombre');
    if (espaciosErrorNombre) errores.nombre_cliente = espaciosErrorNombre;

    ['email_cliente', 'telefono_cliente', 'contraseña_cliente'].forEach(campo => {
        const nombreCampo = campo.replace('_cliente', '');
...
export const validarLogin = (datos) => {
    ...
    if (!datos.email_cliente) {
        errores.email_cliente = 'El email es obligatorio';
    }
    if (!datos.contraseña_cliente) {
        errores.contraseña_cliente = 'La contraseña es obligatoria';
    }
    ...
    if (datos.contraseña_cliente.length === 0) {
        errores.contraseña_cliente = 'La contraseña no puede estar vacía';
    }
...
export const validarProducto = (datos) => {
    ...
    const camposRequeridos = ['nombre_producto', 'descripcion_producto', 'precio_producto', 'id_categoria_producto'];
    camposRequeridos.forEach(campo => {
    ...
    const descripcionCaracteresError = validarCaracteresEspeciales(datos.descripcion_producto, 'descripción');
